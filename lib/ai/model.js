 ...
 *   LLM_MODEL       — Model name override (e.g. "gpt-4o")
...
export async function createModel(options = {}) {
  ...
  const modelName = process.env.LLM_MODEL || DEFAULT_MODELS[provider] || DEFAULT_MODELS.openai;
  ...
  switch (provider) {
    ...
    case 'openai': {
      ...
      const config = { model: modelName, maxTokens };
